...
function setupBlockchainEventListeners() {
  console.log(
    chalk.bgBlue.white(' BLOCKCHAIN ') +
      chalk.cyan(' Setting up event listeners...')
  );

  duelStakingService.setupEventListeners({
    onStakePlaced: async (event) => {
      console.log(
        chalk.bgGreen.black(' STAKE ') +
          ` User ${event.user} staked ${event.amount} for duel ${event.duelId}`
      );

      try {
        // Find the duel by blockchain ID
        const duel = await Duels.findOne({
          blockchainDuelId: Number(event.duelId),
        })
        ...
        if (duel) {
          const isUser1 =
            (duel.user1 as any).walletAddress.toLowerCase() ===
            event.user.toLowerCase();
          const isUser2 =
            (duel.user2 as any).walletAddress.toLowerCase() ===
            event.user.toLowerCase();
    ...
    onDuelSettled: async (event) => {
      console.log(
        chalk.bgYellow.black(' SETTLED ') +
          ` Duel ${event.duelId} settled. Winner: ${event.winner}`
      );
    ...
    onStakeRefunded: async (event) => {
      console.log(
        chalk.bgRed.white(' REFUND ') +
          ` Stake refunded to ${event.user} for duel ${event.duelId}`
      );
...
httpServer.listen(PORT, () => {
  console.log(
    chalk.bgGreen.black(' SUCCESS ') +
      chalk.bold.cyan(' App is listening on Port') +
      chalk.bold.rgb(233, 8, 233).underline(`{PORT}`) +
